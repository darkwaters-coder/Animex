...
const SearchScreen = () => {
  ...
  const handleSearch = async (newQuery) => {
    ...
    try {
      const response = await axios.get(`https://consumet1-sand.vercel.app/anime/gogoanime/${newQuery}?page=${page}`);
  ...
  const handleAnimePress = (id, provider) => {
    if (id && provider) {
      navigation.navigate('Info', { id, provider });
    } else {
      Alert.alert('Error', 'Invalid anime information.');
    }
  };

  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => handleAnimePress(item.id, 'gogoanime')} style={styles.itemContainer}>
